...
const drawing = (e) => {
    ...
    currmouseX = e.offsetX; // salvează poziția mouse-ului curent în variabila "currmouseX".
    currmouseY = e.offsetY; //// salvează poziția mouse-ului curent în variabila "currmouseY".
    puncte.push({ currmouseX, currmouseY });//daugă un obiect cu proprietățile "currmouseX" și "currmouseY" la array-ul "puncte".
...
canvas.addEventListener("mouseup", () => 
{
    ...
    console.log(puncte);
    puncte = [];
    //to implement lot of aglorithms
    
});
*/
function drawLine() {
    ctx.beginPath();
    ctx.moveTo(puncte[0].currmouseX, puncte[0].currmouseY);
    ctx.lineTo(puncte.slice(-1).currmouseX, puncte.slice(-1).currmouseY);
    ctx.stroke();
    snapshot = ctx.getImageData(0, 0, canvas.width, canvas.height);
...
function straightenLine() {
    if (!isDrawing) {
        const deltaX = puncte.slice(-1).currmouseX - prevMouseX;
        const deltaY = puncte.slice(-1).currmouseY - prevMouseY;
        const angle = Math.atan2(deltaY, deltaX);
        const length = Math.sqrt(deltaX * deltaX + deltaY * deltaY);
        endX = prevMouseX + Math.round(Math.cos(angle) * length);
        endY = prevMouseY + Math.round(Math.sin(angle) * length);
        console.log("Verify..")
        drawLine();
    }
}
...
canvas.addEventListener("mouseup", () => //Acesta este un eveniment care se declanșează atunci când utilizatorul ridică butonul mouse-ului de pe suprafața canvas. În acest caz, o funcție anonimă este definită ca răspuns la eveniment.
{
